...
pub fn parse(ua: &str) -> [String; 4] {
    ...
    if let Some(result) = Regex::new("Google.*/\\+/web/snippet").unwrap().captures(ua) {
    ...
    if let Some(result) = Regex::new("(Fennec)/(\\d+)\\.(\\d+)").unwrap().captures(ua) {
        let family = "Firefox Mobile";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(Symphony) (\\d+).(\\d+)").unwrap().captures(ua) {
        let family = result.get(0).map_or_else(|| "", Into::<&str>::into);
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(Blazer)/(\\d+)\\.(\\d+)").unwrap().captures(ua) {
    ...
    if let Some(result) = Regex::new("(ELinks)/(\\d+)\\.(\\d+)").unwrap().captures(ua) {
        let family = result.get(0).map_or_else(|| "", Into::<&str>::into);
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(OLPC)/Update()\\.(\\d+)").unwrap().captures(ua) {
        let family = result.get(0).map_or_else(|| "", Into::<&str>::into);
        let major = "0";
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(PAD-FMD[^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(IBAK\\-[^;/]*) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(?:Cat)(Nova.*) Build").unwrap().captures(ua) {
        let family = "Cat $1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(TPC[0-9]{4,5}) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(Dslide [^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(F\\-\\d[^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(L\\-\\d[^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(N\\-\\d[^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(P\\-\\d[^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(M701|M7|M8|M9) Build").unwrap().captures(ua) {
        let family = "Haipad $1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(DE88Plus|MD70) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *([^;]+) Build/Karbonn").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(Web0S); Linux/(SmartTV)").unwrap().captures(ua) {
        let family = "LG $1 $2";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *MEDION ([^;/]+) Build").unwrap().captures(ua) {
        let family = "Medion $1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *OPPO ?([^;/]+) Build/").unwrap().captures(ua) {
        let family = "Oppo $1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(Xperia [^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(tolino [^;/]+) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new(" (Silk|Kindle)/(\\d+)\\.").unwrap().captures(ua) {
        let family = "Kindle";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("; *(H\\d{2}00\\+?) Build").unwrap().captures(ua) {
        let family = "$1";
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("Kyocera\\-([A-Za-z0-9]+)").unwrap().captures(ua) {
    ...
    if let Some(result) = Regex::new("(Windows NT 6\\.2; ARM;)").unwrap().captures(ua) {
        let family = "Windows";
        let major = "RT";
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(Windows NT 6\\.3; ARM;)").unwrap().captures(ua) {
        let family = "Windows";
        let major = "RT 8.1";
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("^Box.*Windows/([\\d.]+);").unwrap().captures(ua) {
        let family = "Windows";
        let major = "$1";
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(?:PPC|Intel) (Mac OS X)").unwrap().captures(ua) {
        let family = result.get(0).map_or_else(|| "", Into::<&str>::into);
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
    ...
    if let Some(result) = Regex::new("(Linux Mint)(?:/(\\d+))?").unwrap().captures(ua) {
        let family = result.get(0).map_or_else(|| "", Into::<&str>::into);
        let major = result.get(1).map_or_else(|| "0", Into::<&str>::into);
        let minor = result.get(2).map_or_else(|| "0", Into::<&str>::into);
        let patch = result.get(3).map_or_else(|| "0", Into::<&str>::into);
